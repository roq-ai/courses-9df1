...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getCourseUsers() {
    const data = await prisma.course_user
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'course_user'));
  ...
  async function createCourseUser() {
    ...
    const data = await prisma.course_user.create({
      data: body,
    });
    return res.status(200).json(data);
  }
